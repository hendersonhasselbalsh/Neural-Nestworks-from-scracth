...
std::vector<MLP_DATA> LoadData(const std::string& folderPath) {
    ...
    for (const auto& entry : std::filesystem::directory_iterator(folderPath)) {
        if (std::filesystem::is_regular_file(entry.path())) {
            ...
            if (label != l) { 
                l = label; 
                std::cout << "loading data: [" << (label+1)*10 << "%]\n";
            }
        }
    }
...
int main(int argc, const char** argv)
{
    ...
    Gnuplot gnuplot;
    gnuplot.OutFile("D:\\w\\.debugger\\multylayerperceptron\\.resources\\gnuplot-output\\res.dat");
    gnuplot.xRange("0", "");
    ...
    std::vector<MLP_DATA> trainigDataSet  =  LoadData("D:\\w\\.debugger\\multylayerperceptron\\.resources\\train");
    ...
    MLP mlp  =  MlpBuilder()
                    .InputSize(28*28)
                    .Architecture({
                        LayerSignature(100, new Sigmoid(), 0.001),
                        LayerSignature(10, new Sigmoid(), 0.001, new CrossEntropy())
                    })
                    .MaxEpochs(20)
                    .ParseLabelToVector( ParseLabelToEspectedOutput )
                    .SaveOn("D:\\w\\.debugger\\multylayerperceptron\\.resources\\gnuplot-output\\mlp\\mlp.json")
                    .Build();
...
//--- training model, and do a callback on each epoch
    ...
    mlp.Training(trainigDataSet, [&mlp, &trainigDataSet, &ephocCounter, &gnuplot]() {
        ...
        Eigen::MatrixXd confusionMatrix  =  TestingModelAccuracy(&mlp, "D:\\w\\.debugger\\multylayerperceptron\\.resources\\train", &accuracy);
