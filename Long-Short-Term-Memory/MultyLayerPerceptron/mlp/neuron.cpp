    ...
    : _inputSize(inputSize+1), _learningRate(leraningRate), activationFunction(actFun), _lostFunction(lostFunc)
{
    ...
    _lossGradient = std::vector<double>(_inputSize, 1.0);
...
void Neuron::CalculateLossGradient(double correctValue, double predictedValue, std::vector<double> input)
{
    ...
    size_t index = 0;
    for (auto& grad : _lossGradient) {
        grad  =  partialActivated * partialWeightedSum * input[index++];            // (dL/dw) = (dL/da) * (da/dU) * (dU/dw)
    }
...
void Neuron::CalculateLossGradient(double LossPartialWithRespectToActivation, std::vector<double> input)
{
    ...
    size_t index = 0;
    for (auto& grad : _lossGradient) {
        grad  =  LossPartialWithRespectToActivation * partialWeightedSum * input[index++];  // (dL/dw) = (dL/da) * (da/dU) * (dU/dw)
    }
}

double Neuron::LossPartialWithRespectToInput(size_t inputIndex)
{
    return _delta * _weights[inputIndex];      // (dL/dx) = (dL/dU) * (dU/dx) 
}
...
void Neuron::UpdateWeights()
{
    for (size_t i = 0; i < _weights.size(); i++) {
        _weights[i]  =  _weights[i] - _learningRate * _lossGradient[i];
    }
}
...
void Neuron::XavierWeightInitialization(size_t inputSize, size_t outputSize)
{
    ...
    for (auto& weight : _weights) {
        weight  =  Utils::RandomUniformDistribution( -range,range );
    }
}
