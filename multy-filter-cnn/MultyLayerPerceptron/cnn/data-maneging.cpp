...
//
//
//Eigen::Tensor<double, 3> Normalize::Forward(Eigen::Tensor<double, 3>& input)
//{
...
//Eigen::Tensor<double, 3> Normalize::Backward(Eigen::Tensor<double, 3>& dLoss_dOutput)
//{
//	return Eigen::Tensor<double, 3>();
//}
...
Eigen::MatrixXd Normalization::LayerNorm(Eigen::MatrixXd& input, size_t inputIndex)
{
	double epson = 1e-5;

	Eigen::MatrixXd normalized = input;

	_layerMeans[inputIndex] = input.mean();
	double variance = (input.array() - _layerMeans[inputIndex]).square().mean();
	_layerStddev[inputIndex] = std::sqrt(variance + epson);

	normalized = (input.array() - _layerMeans[inputIndex]) / _layerStddev[inputIndex];
...
Eigen::Tensor<double, 3> Normalization::Forward(Eigen::Tensor<double, 3>& input)
{
	_receivecInput  =  input;

	size_t inputQnt  =  _receivecInput.dimension(0);
	size_t rows  =  _receivecInput.dimension(1);
	size_t cols  =  _receivecInput.dimension(2);

	_layerMeans  =  std::vector<double>(inputQnt, 0.0);
	_layerStddev  =  std::vector<double>(inputQnt, 0.0);

	Eigen::Tensor<double, 3> output  =  Eigen::Tensor<double, 3>(inputQnt, rows, cols);

	for (size_t ip = 0; ip < inputQnt; ip++) {
		Eigen::MatrixXd inputMat  =  Utils::TensorSlice(_receivecInput, ip); 
		auto normalizedMat  =  LayerNorm(inputMat, ip);
		output.chip(ip,0)  =  normalizedMat; 
	}
...
Eigen::Tensor<double, 3> Normalization::Backward(Eigen::Tensor<double, 3>& dLoss_dOutput)
{
	size_t inputQnt  =  _receivecInput.dimension(0);
	size_t rows  =  _receivecInput.dimension(1);
	size_t cols  =  _receivecInput.dimension(2);

	Eigen::Tensor<double, 3> dL_dInput  =  Eigen::Tensor<double, 3>(inputQnt, rows, cols);

	for (size_t i = 0; i < inputQnt; i++) {
		Eigen::MatrixXd inputMat  =  Utils::TensorSlice(dLoss_dOutput, i);
		auto dL_dNorm  =  DLoss_DInput(inputMat, i);
		dL_dInput.chip(i, 0)  =  dL_dNorm; 
	}
...
Eigen::MatrixXd Normalization::DLoss_DstdDev(Eigen::MatrixXd& dLoss_dy, size_t index)
{
	double dy_dStdDev  =  -(1.0/std::pow(_layerStddev[index], 2));
	Eigen::MatrixXd dLoss_dStdDev  =  dLoss_dy *  dy_dStdDev;

	return dLoss_dStdDev;
}

Eigen::MatrixXd Normalization::DLoss_DVar(Eigen::MatrixXd& dLoss_dStdDev, size_t index)
{
	double dStdDev_dVar  =  -1.0/(2.0 * _layerStddev[index]);
...
Eigen::MatrixXd Normalization::DLoss_DNii(Eigen::MatrixXd& dLoss_dVar, size_t index)
{
	...
	for (size_t i = 0; i < dLoss_dVar.rows(); i++) {
		for (size_t j = 0; j < dLoss_dVar.cols(); j++) {
			...
			double nii = x - _layerMeans[index];
...
Eigen::MatrixXd Normalization::DLoss_DInput(Eigen::MatrixXd& dLoss_dy, size_t index)
{
	// first route
	double size  =  (double)(dLoss_dy.rows(), dLoss_dy.cols());

	double dy_dNii = 1.0/_layerStddev[index];
	double dNii_dInput  =  1.0 - (1.0/size);
	Eigen::MatrixXd dLoss_dInput = dLoss_dy * dy_dNii * dNii_dInput;


	// second route
	auto dLoss_dStdDev = DLoss_DstdDev(dLoss_dy, index);
	auto dLoss_dVar  =  DLoss_DVar(dLoss_dStdDev, index);
	auto dLoss_dNii  =  DLoss_DNii(dLoss_dVar, index);
