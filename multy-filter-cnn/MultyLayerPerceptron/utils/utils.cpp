...
Eigen::MatrixXd Utils::TensorSlice( Eigen::Tensor<double, 3>& tensor, size_t sliceIndex, size_t dimension)
{
    auto tensorSlice = tensor.chip(sliceIndex, dimension);
    Eigen::Tensor<double, 2> matrixTensor = tensorSlice;

    size_t row  =  matrixTensor.dimension(0);
    size_t col  =  matrixTensor.dimension(1);

    Eigen::Map<Eigen::MatrixXd> matrix(matrixTensor.data(), row, col);
