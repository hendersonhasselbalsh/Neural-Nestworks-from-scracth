...
LayerNormalization::LayerNormalization(double learningRate)
{
...
Eigen::MatrixXd LayerNormalization::Forward(Eigen::MatrixXd& input)
{
    size_t inputCols  =  input.cols();

    if (_betas.empty() == true && _gammas.empty() == true) {
        _gammas  =  std::vector<double>(inputCols, 0.9);
        _betas  =  std::vector<double>(inputCols, 0.00001);
    }

    _layerMeans = std::vector<double>(inputCols, 0.0);
    _layerStddev = std::vector<double>(inputCols, 0.0);
...
Eigen::MatrixXd LayerNormalization::Backward(Eigen::MatrixXd& dL_dNormalized)
{
...
Eigen::MatrixXd LayerNormalization::LayerNorm(Eigen::MatrixXd& input)
{
    double epson = 1e-5;
    ...
    for (size_t col = 0; col < input.cols(); col++) {
        Eigen::VectorXd column = input.col(col);
...
Eigen::MatrixXd LayerNormalization::DL_DVariance(Eigen::MatrixXd& dL_dNormalized)
{
...
Eigen::MatrixXd LayerNormalization::DL_DNii(Eigen::MatrixXd& dL_dVariance, Eigen::MatrixXd& dL_dNormalized)
{
...
Eigen::MatrixXd LayerNormalization::DL_DMeans(Eigen::MatrixXd& dL_dNormalized)
{
...
Eigen::MatrixXd LayerNormalization::DL_DInput(Eigen::MatrixXd& dL_dMean, Eigen::MatrixXd& dL_dNii)
{
